...
double Matchup::CareerTotals::accuracy(int landed, int attempted) const {
    if (attempted == 0) {
...
double Matchup::CareerTotals::perRound(double total) const {
    if (rounds == 0) {
        return std::numeric_limits<double>::quiet_NaN();
    }
...
void Matchup::buildCareerComparisons(const CareerTotals& a, const CareerTotals& b) {
